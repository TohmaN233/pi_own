...
3. 需要布置作业时，在“Assignment 独立工作链”填写名称和要求，为该 Assignment 选择专属资料
   ...
   使用与课程计划相同的数学 Markdown 审阅器展示，并可下载学生版或教师版 Markdown。Agent 生成的
   `.tex`、`.Rmd`、`.md`、`.R` 和 `.txt` 保存在该 Assignment 的 Host 分配目录，可从工作区打开编辑；
   `.tex` 复用课件/讲稿的左右源码与 PDF 编辑器，Ctrl+S 成功编译后才替换旧 PDF。
...
- Assignment：`assignment_state` 只返回一个作业及其专属资料；`read_assignment_material` 必须携带
  ...
  显式回到课程作用域。课程设置中的“Assignment 固定格式”作为每份作业的稳定前言与生成约束，
  无需在每次任务里重复输入，也不会使已批准学期计划过期。教师批准后只保留当前批准的结构化计划/
  课件版本并删除 Assignment 编译辅助文件；已批准源码和当前 PDF 保留。
- 审批：教师 UI 的单独动作；模型工具表中不存在 approve/accept。
- 课件：独立 .tex、实际编译回执、PDF/日志哈希、当前证据接受与恢复。
- 源码审查：固定规则和日志诊断；不是语义正确性证明，也不是截图视觉检查。
- 可视化：默认在已链接课程素材目录中生成自包含交互 HTML，依次用 `add_material` 和 `interactive_visual` 登记；Host 校验控件、Canvas/SVG、依赖和文件 Hash，工作区以独立标签页打开。`visual_templates` / `visual` 仅保留给明确要求的固定 Renderer 图示。当前不自动转图片插入指定 Frame。
