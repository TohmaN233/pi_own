...
import { CourseHost, CourseHostError } from "../../course-host/src/index.ts";
...
function tokenize(text: string): string[] {
	return text
		...
		.match(/[\p{L}\p{N}_]+/gu)?.filter((term) => term.length > 1) ?? [];
}
...
function assertTimestamp(value: string): void {
	if (!Number.isFinite(Date.parse(value))) throw new KnowledgeHostError("INVALID_TIMESTAMP", "Expected ISO-8601 timestamp");
}
...
export class KnowledgeHost {
	...
	search(request: GroundingRequest): GroundingPacket {
		...
		if (!courseVersionId) throw new KnowledgeHostError("COURSE_REQUIRED", "Grounded search requires a course-bound session");
		...
		if (!Number.isSafeInteger(maxHits) || maxHits < 1 || maxHits > 50) throw new KnowledgeHostError("INVALID_LIMIT", "maxHits must be 1..50");
		...
		const ranked = index.spans
			...
			.sort((left, right) => right.score - left.score || left.item.span.spanId.localeCompare(right.item.span.spanId));
	...
	validateDraft(draftValue: unknown, context: DraftValidationContext, checkedAt = new Date().toISOString()): ValidatorResult {
		...
		if (draft.courseVersionId !== courseVersionId) issues.push(issue("COURSE_VERSION_MISMATCH", "Draft targets another course version", "courseVersionId"));
		if (packet) {
			if (packet.courseVersionId !== courseVersionId) issues.push(issue("PACKET_COURSE_MISMATCH", "Packet targets another course version", "packetId"));
			if (packet.sessionBindingId !== context.binding.bindingId) issues.push(issue("PACKET_SESSION_MISMATCH", "Packet belongs to another session binding", "packetId"));
			if (packet.resourceSnapshotId !== context.snapshot.resourceSnapshotId) issues.push(issue("PACKET_SNAPSHOT_MISMATCH", "Packet belongs to another resource snapshot", "packetId"));
		}
		...
		for (const [index, claim] of draft.claims.entries()) {
			...
			if (claim.scope === "derived" && (claim.citationSpanIds.length < 1 || !claim.reason?.trim())) {
				issues.push(issue("DERIVATION_REQUIRED", "Derived claims need source premises and a derivation reason", path));
			}
			if (claim.scope === "computed" && !claim.reason?.trim()) issues.push(issue("COMPUTATION_REASON_REQUIRED", "Computed claims need a computation description", path));
			if (claim.scope === "external") {
				if (context.snapshot.externalKnowledgePolicy === "deny") issues.push(issue("EXTERNAL_KNOWLEDGE_DENIED", "Active profile denies external knowledge", path));
				if (!claim.reason?.trim()) issues.push(issue("EXTERNAL_REASON_REQUIRED", "External claims need a reason and source guidance", path));
			}
			if (claim.scope === "insufficient" && !claim.reason?.trim()) issues.push(issue("INSUFFICIENT_REASON_REQUIRED", "Insufficient-evidence claims need an explanation", path));
			for (const spanId of new Set(claim.citationSpanIds)) {
				const cited = packetSpans.get(spanId);
				if (!cited) {
					issues.push(issue("FORGED_CITATION", `Citation ${spanId} is not in the issued Grounding Packet`, `${path}.citationSpanIds`));
					continue;
				}
				try {
					const current = this.courseHost.readSpan(packet?.courseVersionId ?? "", spanId);
					if (current.textHash !== cited.textHash) issues.push(issue("STALE_CITATION", `Citation ${spanId} no longer matches source content`, `${path}.citationSpanIds`));
				} catch (error) {
	...
	publishDraft(draftValue: unknown, context: DraftValidationContext, publishedAt = new Date().toISOString()): PublicationReceipt {
		...
		this.drafts.set(draft.draftId, Object.freeze({ ...draft, claims: draft.claims.map((claim) => ({ ...claim, citationSpanIds: [...claim.citationSpanIds] })) }));
	...
	exportState(): KnowledgeHostState {
		return {
			...
			publications: [...this.receipts.values()]
				...
				.map((receipt) => {
					const draft = this.drafts.get(receipt.draftId);
					if (!draft) throw new KnowledgeHostError("CORRUPT_STATE", `Missing draft ${receipt.draftId}`);
	...
	restoreState(state: KnowledgeHostState): void {
		...
		if (this.packets.size || this.receipts.size || this.drafts.size) throw new KnowledgeHostError("STATE_NOT_EMPTY", "KnowledgeHost restore requires an empty host");
		for (const packet of state.packets) {
			this.courseHost.getVersion(packet.courseVersionId);
			if (`sha256:${sha256Hex(packet.query)}` !== packet.queryHash) throw new KnowledgeHostError("PACKET_HASH_MISMATCH", `Packet ${packet.packetId} has an invalid query hash`);
			for (const span of packet.spans) {
				const current = this.courseHost.readSpan(packet.courseVersionId, span.spanId);
				if (current.textHash !== span.textHash || current.text !== span.text) throw new KnowledgeHostError("PACKET_SPAN_MISMATCH", `Packet ${packet.packetId} has stale span ${span.spanId}`);
			}
			const identity = {
			...
			if (contentHash(identity) !== packet.contentHash || deterministicId("grounding", identity, 32) !== packet.packetId) {
				throw new KnowledgeHostError("PACKET_INTEGRITY_FAILURE", `Packet ${packet.packetId} failed integrity validation`);
			}
		...
		for (const publication of state.publications) {
			...
			if (contentHash(identity) !== receipt.contentHash || deterministicId("publication", identity, 32) !== receipt.receiptId) {
				throw new KnowledgeHostError("PUBLICATION_INTEGRITY_FAILURE", `Publication ${receipt.receiptId} failed integrity validation`);
			}
			this.drafts.set(draft.draftId, Object.freeze({ ...draft, claims: draft.claims.map((claim) => ({ ...claim, citationSpanIds: [...claim.citationSpanIds] })) }));
			this.receipts.set(receipt.receiptId, Object.freeze({ ...receipt }));
		}
	}
}
