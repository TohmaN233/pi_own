...
import { parseVisualizationSpec, type JsonValue } from "../../harness-contracts/src/index.ts";
...
function stringValue(value: unknown, path: string, max = 20_000): string {
	if (typeof value !== "string" || !value.trim()) throw new CourseBuilderError("INVALID_INPUT", `${path} must be a non-empty string`);
...
function timestamp(value: string, path: string): string {
	if (!Number.isFinite(Date.parse(value))) throw new CourseBuilderError("INVALID_TIMESTAMP", `${path} must be ISO-8601`);
	return value;
}

function stringArray(value: unknown, path: string, maxItems = 512): string[] {
	if (!Array.isArray(value) || value.length > maxItems) throw new CourseBuilderError("INVALID_INPUT", `${path} must be an array of at most ${maxItems} strings`);
...
function jsonRecord(value: unknown, path: string): Record<string, JsonValue> {
	...
	if (encoded.length > 200_000) throw new CourseBuilderError("INPUT_TOO_LARGE", `${path} exceeds 200000 serialized characters`);
	return JSON.parse(encoded) as Record<string, JsonValue>;
}

function unique(values: readonly string[], path: string): string[] {
	if (new Set(values).size !== values.length) throw new CourseBuilderError("DUPLICATE_VALUE", `${path} contains duplicate values`);
	return [...values];
}
...
export function assertNoAgentApprovalFields(value: unknown, path = "draft"): void {
	if (Array.isArray(value)) {
		value.forEach((item, index) => assertNoAgentApprovalFields(item, `${path}[${index}]`));
		return;
	}
...
function parseBeamerProfile(value: unknown): BeamerProfile {
	...
	exactKeys(input, ["aspectRatio", "fontSize", "theme", "author", "institute", "language", "overlayPolicy", "referencesPolicy", "backupSlides", "speakerNotes", "preamble"], "beamerProfile");
	if (input.aspectRatio !== "169" && input.aspectRatio !== "43") throw new CourseBuilderError("INVALID_INPUT", "beamerProfile.aspectRatio must be 169 or 43");
	if (input.overlayPolicy !== "allow" && input.overlayPolicy !== "deny") throw new CourseBuilderError("INVALID_INPUT", "beamerProfile.overlayPolicy is invalid");
	if (input.referencesPolicy !== "required" && input.referencesPolicy !== "optional") throw new CourseBuilderError("INVALID_INPUT", "beamerProfile.referencesPolicy is invalid");
	if (typeof input.speakerNotes !== "boolean") throw new CourseBuilderError("INVALID_INPUT", "beamerProfile.speakerNotes must be boolean");
	return {
		...
		author: stringValue(input.author, "beamerProfile.author", 256),
		institute: stringValue(input.institute, "beamerProfile.institute", 256),
...
export function parseCourseBuilderProjectInput(value: unknown): CourseBuilderProjectInput {
	...
	exactKeys(input, ["courseId", "title", "weeks", "sessionsPerWeek", "minutesPerSession", "audience", "language", "goals", "beamerProfile"], "project");
	const courseId = stringValue(input.courseId, "project.courseId", 128);
	if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/u.test(courseId)) throw new CourseBuilderError("INVALID_COURSE_ID", "project.courseId contains unsupported characters");
	return {
		courseId,
...
function parseSpiralRevisit(value: unknown, path: string) {
	...
	const allowed = new Set(["complexity", "relationship", "abstraction", "formalization", "representation", "transfer", "boundary"]);
	if (typeof input.progression !== "string" || !allowed.has(input.progression)) throw new CourseBuilderError("INVALID_INPUT", `${path}.progression is invalid`);
	return {
		conceptId: stringValue(input.conceptId, `${path}.conceptId`, 256),
		progression: input.progression as "complexity" | "relationship" | "abstraction" | "formalization" | "representation" | "transfer" | "boundary",
...
export function parseSemesterPlanDraft(value: unknown): SemesterPlanDraft {
	...
	if (!Array.isArray(input.sessions) || input.sessions.length === 0 || input.sessions.length > 840) throw new CourseBuilderError("INVALID_INPUT", "semesterPlan.sessions must have 1..840 entries");
	return {
		...
		sessions: input.sessions.map((value, index) => {
			...
			exactKeys(session, ["week", "session", "title", "objectives", "prerequisites", "topics", "materialIds", "activities", "understandingEvidence", "assessment", "homework", "courseGoalsCovered", "revisits", "visualOpportunities"], path);
			if (!Array.isArray(session.revisits)) throw new CourseBuilderError("INVALID_INPUT", `${path}.revisits must be an array`);
			return {
				...
				prerequisites: unique(stringArray(session.prerequisites, `${path}.prerequisites`, 50), `${path}.prerequisites`),
				...
				courseGoalsCovered: unique(stringArray(session.courseGoalsCovered, `${path}.courseGoalsCovered`, 100), `${path}.courseGoalsCovered`),
				revisits: session.revisits.map((item, revisitIndex) => parseSpiralRevisit(item, `${path}.revisits[${revisitIndex}]`)),
...
export function parseLessonPlanDraft(value: unknown): LessonPlanDraft {
	...
	exactKeys(input, ["week", "session", "title", "objectives", "prerequisites", "misconceptions", "segments", "examples", "exercises", "materialIds", "visualRequests", "notes"], "lessonPlan");
	if (!Array.isArray(input.segments) || input.segments.length === 0 || input.segments.length > 100) throw new CourseBuilderError("INVALID_INPUT", "lessonPlan.segments must have 1..100 entries");
	return {
		...
		prerequisites: unique(stringArray(input.prerequisites, "lessonPlan.prerequisites", 50), "lessonPlan.prerequisites"),
		...
		segments: input.segments.map((value, index) => {
			...
			return {
				...
				checkForUnderstanding: nullableString(segment.checkForUnderstanding, `${path}.checkForUnderstanding`, 10_000),
			};
		}),
...
export function parseBeamerDeckDraft(value: unknown): BeamerDeckDraft {
	...
	return {
		...
		assetMaterialIds: unique(stringArray(input.assetMaterialIds, "beamerDeck.assetMaterialIds", 500), "beamerDeck.assetMaterialIds"),
	};
}

export function parseMaterialAnalysis(value: unknown): Omit<MaterialAnalysis, "projectId" | "materialIds" | "createdAt" | "contentHash"> {
	assertNoAgentApprovalFields(value, "materialAnalysis");
	const input = requireRecord(value, "materialAnalysis");
	exactKeys(input, ["topicChains", "prerequisiteGaps", "duplicates", "sequenceGaps", "terminologyConflicts", "practiceOpportunities", "visualOpportunities"], "materialAnalysis");
	return {
...
function assertHash(value: { contentHash: string }, payload: unknown, label: string): void {
	if (value.contentHash !== contentHash(payload)) throw new CourseBuilderError("CORRUPT_STATE", `${label} has an invalid content hash`);
}
...
export class CourseBuilderHost {
	...
	createProject(value: unknown, createdAt = new Date().toISOString()): CourseBuilderProject {
		...
		if (this.projects.size >= MAX_PROJECTS) throw new CourseBuilderError("PROJECT_LIMIT", `Course Builder supports at most ${MAX_PROJECTS} projects`);
		...
		const base: Omit<CourseBuilderProject, "contentHash"> = { ...input, projectId, revision: 1, createdAt, updatedAt: createdAt };
		const project: CourseBuilderProject = { ...base, contentHash: contentHash(base) };
		this.mutate(() => this.projects.set(projectId, project));
		return clone(project);
	}

	updateProject(projectId: string, value: unknown, expectedRevision: number, updatedAt = new Date().toISOString()): CourseBuilderProject {
		const current = this.requireProject(projectId);
		if (current.revision !== expectedRevision) throw new CourseBuilderError("REVISION_CONFLICT", `Expected project revision ${expectedRevision}, actual ${current.revision}`);
		const input = parseCourseBuilderProjectInput(value);
		if (input.courseId !== current.courseId) throw new CourseBuilderError("COURSE_ID_IMMUTABLE", "project.courseId cannot change");
		timestamp(updatedAt, "updatedAt");
		const base: Omit<CourseBuilderProject, "contentHash"> = { ...input, projectId, revision: current.revision + 1, createdAt: current.createdAt, updatedAt };
	...
	bindSession(sessionId: string, projectId: string, boundAt = new Date().toISOString()): SessionProjectBinding {
		...
		if (current) {
			if (current.projectId !== projectId) throw new CourseBuilderError("SESSION_REBIND_FORBIDDEN", "A Pi session cannot be silently rebound to another Course Builder project");
	...
	listProjects(): CourseBuilderProject[] {
		return [...this.projects.values()].sort((left, right) => right.updatedAt.localeCompare(left.updatedAt)).map(clone);
	}
	...
	importMaterials(sessionId: string, values: readonly CourseBuilderMaterialInput[], expectedProjectRevision: number, createdAt = new Date().toISOString()): CourseBuilderMaterial[] {
		const project = this.requireProjectForSession(sessionId);
		if (project.revision !== expectedProjectRevision) throw new CourseBuilderError("REVISION_CONFLICT", `Expected project revision ${expectedProjectRevision}, actual ${project.revision}`);
		timestamp(createdAt, "createdAt");
		if (!Array.isArray(values) || values.length === 0 || values.length > 100) throw new CourseBuilderError("INVALID_INPUT", "Material batch must contain 1..100 files");
		const existing = this.projectMaterials(project.projectId);
		if (existing.length + values.length > MAX_MATERIALS_PER_PROJECT) throw new CourseBuilderError("MATERIAL_LIMIT", `Project supports at most ${MAX_MATERIALS_PER_PROJECT} materials`);
		const existingNames = new Set(existing.map((item) => item.name));
		const batchNames = new Set<string>();
		const prepared = values.map((value, index) => {
			const name = stringValue(value.name, `materials[${index}].name`, 512);
			if (!new Set(["pptx", "pdf", "tex", "markdown", "text"]).has(value.kind)) throw new CourseBuilderError("INVALID_INPUT", `materials[${index}].kind is invalid`);
			if (!(value.sourceBytes instanceof Uint8Array)) throw new CourseBuilderError("INVALID_INPUT", `materials[${index}].sourceBytes must be Uint8Array`);
			if (value.sourceBytes.byteLength > MAX_MATERIAL_INPUT_BYTES) throw new CourseBuilderError("MATERIAL_TOO_LARGE", `${name} exceeds ${MAX_MATERIAL_INPUT_BYTES} bytes`);
			if (typeof value.extractedText !== "string" || value.extractedText.length > MAX_MATERIAL_TEXT_CHARACTERS) throw new CourseBuilderError("MATERIAL_TEXT_TOO_LARGE", `${name} exceeds ${MAX_MATERIAL_TEXT_CHARACTERS} extracted characters`);
			if (existingNames.has(name) || batchNames.has(name)) throw new CourseBuilderError("DUPLICATE_MATERIAL", `Material ${name} already exists in the project or batch`);
			batchNames.add(name);
		...
		const updatedPayload: Omit<CourseBuilderProject, "contentHash"> = { ...currentPayload, revision: project.revision + 1, updatedAt: createdAt };
	...
	saveMaterialAnalysis(sessionId: string, value: unknown, createdAt = new Date().toISOString()): MaterialAnalysis {
		...
		timestamp(createdAt, "createdAt");
		const materialIds = this.projectMaterials(project.projectId).map((item) => item.materialId).sort();
		if (materialIds.length === 0) throw new CourseBuilderError("MATERIALS_REQUIRED", "Import course materials before saving an analysis");
	...
	saveSemesterPlan(sessionId: string, value: unknown, createdAt = new Date().toISOString()): SemesterPlan {
		const project = this.requireProjectForSession(sessionId);
	...
	reviewSemesterPlan(sessionId: string, planId: string, expectedRevision: number, decision: ReviewDecision, note: string, reviewedAt = new Date().toISOString()): SemesterPlan {
		const project = this.requireProjectForSession(sessionId);
		const history = this.semesterPlans.get(project.projectId) ?? [];
		const current = currentByRevision(history);
		if (!current || current.semesterPlanId !== planId) throw new CourseBuilderError("SEMESTER_PLAN_NOT_FOUND", "Current Semester Plan was not found");
		if (current.revision !== expectedRevision) throw new CourseBuilderError("REVISION_CONFLICT", `Expected Semester Plan revision ${expectedRevision}, actual ${current.revision}`);
		const review = this.teacherReview(decision, note, expectedRevision, current.contentHash, reviewedAt);
		const base: Omit<SemesterPlan, "contentHash"> = { ...current, status: decision === "approve" ? "approved" : "changes-requested", review, updatedAt: reviewedAt };
		const next: SemesterPlan = { ...base, contentHash: contentHash(base) };
		this.mutate(() => this.semesterPlans.set(project.projectId, [...history.filter((item) => item.revision !== current.revision), next]));
		return clone(next);
	}

	saveLessonPlan(sessionId: string, value: unknown, createdAt = new Date().toISOString()): LessonPlan {
		const project = this.requireProjectForSession(sessionId);
		const semester = this.currentSemesterPlan(project.projectId);
		if (!semester || semester.status !== "approved") throw new CourseBuilderError("SEMESTER_APPROVAL_REQUIRED", "Approve the current Semester Plan before creating Lesson Plans");
		...
		if (!slot) throw new CourseBuilderError("SEMESTER_SLOT_REQUIRED", "Lesson Plan does not correspond to a Semester Plan session");
		const minutes = draft.segments.reduce((sum, segment) => sum + segment.minutes, 0);
		if (minutes > project.minutesPerSession) throw new CourseBuilderError("LESSON_TIME_EXCEEDED", `Lesson segments total ${minutes} minutes, exceeding ${project.minutesPerSession}`);
		this.assertMaterials(project.projectId, draft.materialIds);
		const lessonId = deterministicId("lesson-plan", { projectId: project.projectId, week: draft.week, session: draft.session }, 40);
	...
	reviewLessonPlan(sessionId: string, lessonPlanId: string, expectedRevision: number, decision: ReviewDecision, note: string, reviewedAt = new Date().toISOString()): LessonPlan {
		const project = this.requireProjectForSession(sessionId);
		const history = this.lessonPlans.get(lessonPlanId) ?? [];
		const current = currentByRevision(history);
		if (!current || current.projectId !== project.projectId) throw new CourseBuilderError("LESSON_PLAN_NOT_FOUND", "Current Lesson Plan was not found");
		if (current.revision !== expectedRevision) throw new CourseBuilderError("REVISION_CONFLICT", `Expected Lesson Plan revision ${expectedRevision}, actual ${current.revision}`);
		const review = this.teacherReview(decision, note, expectedRevision, current.contentHash, reviewedAt);
		const base: Omit<LessonPlan, "contentHash"> = { ...current, status: decision === "approve" ? "approved" : "changes-requested", review, updatedAt: reviewedAt };
		const next: LessonPlan = { ...base, contentHash: contentHash(base) };
		this.mutate(() => this.lessonPlans.set(lessonPlanId, [...history.filter((item) => item.revision !== current.revision), next]));
		return clone(next);
	}

	saveBeamerDeck(sessionId: string, value: unknown, createdAt = new Date().toISOString()): BeamerDeck {
		const project = this.requireProjectForSession(sessionId);
		const draft = parseBeamerDeckDraft(value);
		const lesson = this.currentLessonPlan(draft.lessonPlanId);
		if (!lesson || lesson.projectId !== project.projectId) throw new CourseBuilderError("LESSON_PLAN_NOT_FOUND", "Deck Lesson Plan was not found in this project");
		if (lesson.status !== "approved") throw new CourseBuilderError("LESSON_APPROVAL_REQUIRED", "Approve the current Lesson Plan before creating a Beamer deck");
		this.assertMaterials(project.projectId, draft.assetMaterialIds);
		timestamp(createdAt, "createdAt");
		const deckId = deterministicId("beamer-deck", { projectId: project.projectId, lessonPlanId: lesson.lessonPlanId }, 40);
	...
	recordCompile(sessionId: string, receipt: BeamerCompileReceipt, artifact: BeamerCompiledArtifact | null): BeamerCompileReceipt {
		const project = this.requireProjectForSession(sessionId);
		const deck = this.currentDeck(receipt.deckId);
		if (!deck || deck.projectId !== project.projectId) throw new CourseBuilderError("DECK_NOT_FOUND", "Compile receipt deck was not found");
		if (receipt.deckRevision !== deck.revision || receipt.sourceHash !== deck.sourceHash) throw new CourseBuilderError("STALE_COMPILE_RECEIPT", "Compile receipt does not match the current deck revision");
		const { contentHash: _hash, ...payload } = receipt;
		assertHash(receipt, payload, "Compile receipt");
		if (artifact && artifact.receiptId !== receipt.receiptId) throw new CourseBuilderError("COMPILE_ARTIFACT_MISMATCH", "Compiled PDF belongs to another receipt");
		const history = this.decks.get(deck.deckId) ?? [];
		const base: Omit<BeamerDeck, "contentHash"> = { ...deck, status: receipt.succeeded ? "compiled" : "draft", updatedAt: receipt.createdAt };
	...
	recordDeckReview(sessionId: string, review: DeckReview): DeckReview {
		...
		if (!deck || deck.projectId !== project.projectId) throw new CourseBuilderError("DECK_NOT_FOUND", "Deck review target was not found");
		if (review.deckRevision !== deck.revision || review.sourceHash !== deck.sourceHash) throw new CourseBuilderError("STALE_DECK_REVIEW", "Review does not match the current deck revision");
		const { contentHash: _hash, ...payload } = review;
		assertHash(review, payload, "Deck review");
		const history = this.decks.get(deck.deckId) ?? [];
		const base: Omit<BeamerDeck, "contentHash"> = { ...deck, status: review.status === "pass" ? "reviewed" : deck.status, updatedAt: review.createdAt };
		const updated: BeamerDeck = { ...base, contentHash: contentHash(base) };
		this.mutate(() => {
	...
	acceptDeck(sessionId: string, deckId: string, expectedRevision: number, compileReceiptId: string, reviewId: string, acceptedAt = new Date().toISOString()): BeamerDeck {
		const project = this.requireProjectForSession(sessionId);
		const deck = this.currentDeck(deckId);
		if (!deck || deck.projectId !== project.projectId) throw new CourseBuilderError("DECK_NOT_FOUND", "Deck was not found");
		if (deck.revision !== expectedRevision) throw new CourseBuilderError("REVISION_CONFLICT", `Expected Deck revision ${expectedRevision}, actual ${deck.revision}`);
		const receipt = this.compileReceipts.get(compileReceiptId);
		const review = this.deckReviews.get(reviewId);
		if (!receipt || !receipt.succeeded || receipt.deckId !== deckId || receipt.deckRevision !== deck.revision || receipt.sourceHash !== deck.sourceHash) throw new CourseBuilderError("VALID_COMPILE_REQUIRED", "A successful current compile receipt is required");
		if (!review || review.status !== "pass" || review.deckId !== deckId || review.deckRevision !== deck.revision || review.sourceHash !== deck.sourceHash || review.compileReceiptId !== receipt.receiptId) throw new CourseBuilderError("VALID_REVIEW_REQUIRED", "A passing current deck review is required");
		timestamp(acceptedAt, "acceptedAt");
		const history = this.decks.get(deckId) ?? [];
		const base: Omit<BeamerDeck, "contentHash"> = { ...deck, status: "accepted", acceptedAt, acceptedReceiptId: receipt.receiptId, updatedAt: acceptedAt };
		const accepted: BeamerDeck = { ...base, contentHash: contentHash(base) };
		this.mutate(() => this.decks.set(deckId, [...history.filter((item) => item.revision !== deck.revision), accepted]));
		return clone(accepted);
	}

	createVisual(sessionId: string, lessonPlanId: string, specValue: unknown, learningPurpose: string, createdAt = new Date().toISOString()): CourseBuilderVisual {
		const project = this.requireProjectForSession(sessionId);
		const lesson = this.currentLessonPlan(lessonPlanId);
		if (!lesson || lesson.projectId !== project.projectId) throw new CourseBuilderError("LESSON_PLAN_NOT_FOUND", "Visual Lesson Plan was not found");
		const spec = parseVisualizationSpec(specValue);
		if (spec.courseVersionId !== project.projectId) throw new CourseBuilderError("VISUAL_PROJECT_MISMATCH", "Course Builder visual spec must use the projectId as its bounded owner id");
		...
		if (validation.status !== "pass") throw new CourseBuilderError("VISUAL_VALIDATION_FAILED", validation.issues.map((item) => item.message).join("; "));
		const published = this.visualHost.publish(artifact.artifactId);
		const base = { projectId: project.projectId, lessonPlanId, spec, artifact: published, learningPurpose: purpose, createdAt };
		const visual: CourseBuilderVisual = { visualId: deterministicId("course-builder-visual", base, 40), ...base, contentHash: contentHash(base) };
	...
	getCompiledPdf(sessionId: string, receiptId: string): Uint8Array {
		...
		if (!receipt || receipt.projectId !== project.projectId) throw new CourseBuilderError("COMPILE_RECEIPT_NOT_FOUND", "Compile receipt was not found in this project");
		const row = this.database.prepare("SELECT bytes FROM course_builder_pdf WHERE receipt_id = ?").get(receiptId) as { bytes?: Uint8Array } | undefined;
	...
	exportState(): CourseBuilderState {
		return {
			version: 1,
			projects: [...this.projects.values()].sort((left, right) => left.projectId.localeCompare(right.projectId)).map(clone),
			bindings: [...this.bindings.values()].sort((left, right) => left.sessionId.localeCompare(right.sessionId)).map(clone),
			materials: [...this.materials.values()].sort((left, right) => left.materialId.localeCompare(right.materialId)).map(clone),
			materialAnalyses: [...this.materialAnalyses.values()].sort((left, right) => left.projectId.localeCompare(right.projectId)).map(clone),
			semesterPlans: [...this.semesterPlans.values()].flat().sort((left, right) => left.projectId.localeCompare(right.projectId) || left.revision - right.revision).map(clone),
			lessonPlans: [...this.lessonPlans.values()].flat().sort((left, right) => left.lessonPlanId.localeCompare(right.lessonPlanId) || left.revision - right.revision).map(clone),
			decks: [...this.decks.values()].flat().sort((left, right) => left.deckId.localeCompare(right.deckId) || left.revision - right.revision).map(clone),
			compileReceipts: [...this.compileReceipts.values()].sort((left, right) => left.receiptId.localeCompare(right.receiptId)).map(clone),
			deckReviews: [...this.deckReviews.values()].sort((left, right) => left.reviewId.localeCompare(right.reviewId)).map(clone),
			visuals: [...this.visuals.values()].sort((left, right) => left.visualId.localeCompare(right.visualId)).map(clone),
		};
	}
	...
	private validateSemesterPlan(project: CourseBuilderProject, plan: SemesterPlanDraft): void {
		...
		if (plan.sessions.length !== capacity) throw new CourseBuilderError("SEMESTER_CAPACITY_MISMATCH", `Semester Plan must explicitly contain ${capacity} session slots, including breaks or exams`);
		const slots = new Set<string>();
		const goals = new Set<string>();
		for (const session of plan.sessions) {
			if (session.week > project.weeks || session.session > project.sessionsPerWeek) throw new CourseBuilderError("SEMESTER_SLOT_OUT_OF_RANGE", `Week ${session.week}, session ${session.session} is outside the project calendar`);
		...
		for (const goal of project.goals) if (!goals.has(goal)) throw new CourseBuilderError("COURSE_GOAL_UNCOVERED", `Semester Plan does not cover project goal: ${goal}`);
	}

	private teacherReview(decision: ReviewDecision, note: string, targetRevision: number, targetHash: string, reviewedAt: string): TeacherReview {
		if (decision !== "approve" && decision !== "request-changes") throw new CourseBuilderError("INVALID_REVIEW_DECISION", "Teacher review decision is invalid");
		return {
			decision,
	...
	private snapshot(projectId: string): CourseBuilderSnapshot {
		...
		const lessons = [...this.lessonPlans.values()].map((items) => currentByRevision(items)).filter((item): item is LessonPlan => item !== null && item.projectId === projectId);
		const decks = [...this.decks.values()].map((items) => currentByRevision(items)).filter((item): item is BeamerDeck => item !== null && item.projectId === projectId);
		return {
			...
			compileReceipts: [...this.compileReceipts.values()].filter((item) => item.projectId === projectId).sort((left, right) => left.createdAt.localeCompare(right.createdAt)).map(clone),
			deckReviews: [...this.deckReviews.values()].filter((item) => item.projectId === projectId).sort((left, right) => left.createdAt.localeCompare(right.createdAt)).map(clone),
			visuals: [...this.visuals.values()].filter((item) => item.projectId === projectId).sort((left, right) => left.createdAt.localeCompare(right.createdAt)).map(clone),
		};
	}

	private projectMaterials(projectId: string): CourseBuilderMaterial[] {
		return [...this.materials.values()].filter((item) => item.projectId === projectId).sort((left, right) => left.createdAt.localeCompare(right.createdAt) || left.name.localeCompare(right.name));
	}

	private assertMaterials(projectId: string, materialIds: readonly string[]): void {
		for (const materialId of materialIds) {
			const material = this.materials.get(materialId);
			if (!material || material.projectId !== projectId) throw new CourseBuilderError("MATERIAL_NOT_FOUND", `Material ${materialId} is not available in this project`);
		}
	}
	...
	private requireProjectForSession(sessionId: string): CourseBuilderProject {
		const binding = this.bindings.get(sessionId);
		if (!binding) throw new CourseBuilderError("PROJECT_BINDING_REQUIRED", "Bind this Pi session to a Course Builder project first");
		return this.requireProject(binding.projectId);
	}

	private mutate(change: () => void, artifact?: BeamerCompiledArtifact): void {
	...
	private persist(artifact?: BeamerCompiledArtifact): void {
		const json = stableStringify(this.exportState());
		this.database.exec("BEGIN IMMEDIATE");
		try {
			this.database.prepare("INSERT INTO course_builder_state(key, value) VALUES(?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value").run(STATE_KEY, json);
			if (artifact) this.database.prepare("INSERT INTO course_builder_pdf(receipt_id, bytes) VALUES(?, ?) ON CONFLICT(receipt_id) DO UPDATE SET bytes = excluded.bytes").run(artifact.receiptId, artifact.pdfBytes);
	...
	private restore(): void {
		const row = this.database.prepare("SELECT value FROM course_builder_state WHERE key = ?").get(STATE_KEY) as { value?: string } | undefined;
		if (!row?.value) return;
		let state: unknown;
		try {
	...
	private validateState(value: unknown): CourseBuilderState {
		...
		if (state.version !== 1) throw new CourseBuilderError("CORRUPT_STATE", "Course Builder state version is unsupported");
		for (const key of ["projects", "bindings", "materials", "materialAnalyses", "semesterPlans", "lessonPlans", "decks", "compileReceipts", "deckReviews", "visuals"]) {
			if (!Array.isArray(state[key])) throw new CourseBuilderError("CORRUPT_STATE", `Course Builder state ${key} must be an array`);
		}
		...
		for (const material of result.materials) {
			if (`sha256:${sha256Hex(material.extractedText)}` !== material.textHash) throw new CourseBuilderError("CORRUPT_STATE", `Material ${material.materialId} has invalid text hash`);
		}
	...
	private loadState(state: CourseBuilderState): void {
		...
		for (const item of state.semesterPlans) this.semesterPlans.set(item.projectId, [...(this.semesterPlans.get(item.projectId) ?? []), clone(item)]);
		this.lessonPlans = new Map();
		for (const item of state.lessonPlans) this.lessonPlans.set(item.lessonPlanId, [...(this.lessonPlans.get(item.lessonPlanId) ?? []), clone(item)]);
		this.decks = new Map();
		for (const item of state.decks) this.decks.set(item.deckId, [...(this.decks.get(item.deckId) ?? []), clone(item)]);
