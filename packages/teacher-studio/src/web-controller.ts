...
import type { TeacherStudio } from "./teacher-studio.ts";
...
function object(value: unknown): Record<string, unknown> {
	if (!value || typeof value !== "object" || Array.isArray(value))
		throw new Error("Teacher request input must be an object");
	...
	async dispatch(actionValue: unknown, inputValue: unknown): Promise<TeacherWebResponse> {
		try {
			...
			if (action === "create-draft")
				return {
					ok: true,
					data: this.studio.createDraft(string(input.courseId, "courseId"), string(input.title, "title")),
				};
			...
			if (action === "publish") {
				if (!Array.isArray(input.profiles) || !input.profiles.every((value) => typeof value === "string"))
					throw new Error("profiles must be a string array");
				return {
					ok: true,
			...
			return {
				ok: false,
				error: { code, message: error instanceof Error ? error.message : "Teacher action failed" },
			};
		}
	}
}
...
export function createTeacherRouteHandler(
	...
	return async (request: Request): Promise<Response> => {
		if (!(await authorizeTeacher(request)))
			return Response.json(
				{ ok: false, error: { code: "TEACHER_REQUIRED", message: "Teacher authorization required" } },
				{ status: 403 },
			);
		...
		try {
			...
			return Response.json(
				{ ok: false, error: { code: "INVALID_JSON", message: "Malformed JSON" } },
				{ status: 400 },
			);
		}
		const input = object(body);
		for (const key of Object.keys(input))
			if (key !== "action" && key !== "input")
				return Response.json(
					{ ok: false, error: { code: "UNKNOWN_FIELD", message: `Unknown field ${key}` } },
					{ status: 400 },
				);
