...
export async function POST(request: Request) {
	...
	try {
		...
		const receiptId = builderString(body.receiptId);
		...
		if (body.line !== undefined) {
			if (
				...
				throw new Error("定位需要有效源码行号，不能同时指定 PDF 位置。");
			...
			if (
				...
				![body.x, body.y].every((value) => Number.isFinite(value) && value >= 0 && value <= 100_000)
			)
				throw new Error("定位需要有效 PDF 页码和坐标。");
			query = { page: body.page as number, x: body.x, y: body.y };
		}
		const location = await getCourseBuilderHost().locateBeamer(sessionId, receiptId, query);
		console.info("[course-builder] Beamer SyncTeX", { sessionId, receiptId, query, location });
