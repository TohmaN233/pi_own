...
import { courseBuilderInteractiveVisual, getCourseBuilderHost } from "@/lib/course-builder-service";
...
export async function GET(request:Request) {
 ...
 try {
  ...
  const host=getCourseBuilderHost(),kind=q.get("kind");let bytes:Uint8Array|string,filename:string,type:string,disposition="attachment",interactiveVisual=false;
  ...
  else if(kind==="visual") {const v=host.getSnapshotForSession(sid)?.visuals.find(v=>v.visualId===id);if(!v)throw new Error("No visual in this project");interactiveVisual=v.format==="interactive-html";bytes=interactiveVisual ? await courseBuilderInteractiveVisual(sid,id) : v.artifact?.html ?? (()=>{throw new Error("Visual artifact is unavailable")})();filename=interactiveVisual ? `${(v.title || "interactive-visual").replace(/[^\p{L}\p{N}_.-]+/gu,"-")}.html` : "visual.html";type="text/html; charset=utf-8";disposition="inline";}
  ...
  return new Response(typeof bytes==="string"?bytes:new Uint8Array(bytes),{headers:{"content-type":type,"content-disposition":`${disposition}; filename="${filename}"`,"cache-control":"no-store","x-content-type-options":"nosniff",...(kind==="visual"?{"content-security-policy":interactiveVisual ? "default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src data: blob:; font-src data:; media-src data: blob:; connect-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'" : "default-src 'none'; style-src 'unsafe-inline'; img-src data:; base-uri 'none'; form-action 'none'; frame-ancestors 'none'"}:{})}});
