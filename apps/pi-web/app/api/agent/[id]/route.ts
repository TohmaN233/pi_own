...
export async function POST(
  ...
  try {
    ...
    if (existing?.isAlive()) {
      reconcileHarnessSession(existing.inner.sessionManager);
      const result = await existing.send(body);
      await inheritHarnessReplacement(existing.inner.sessionManager, body.type, result);
    ...
    await inheritHarnessReplacement(session.inner.sessionManager, body.type, result);
    promptAccepted = body.type === "prompt";

    return NextResponse.json({ success: true, data: result });
  } catch (error) {
    console.error("[pi-web] Harness-aware agent command failed", { sessionId: id, commandType, error });
    return NextResponse.json({
...
export async function GET(
  ...
  try {
    ...
    console.error("[pi-web] failed to read Harness-aware agent state", { sessionId: id, error });
