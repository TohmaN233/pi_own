      ...
      <section className={styles.content}>
        ...
        {!data ? <p>正在读取项目…</p> : <>
          ...
          <ul className={styles.list}>{conversations.map((item) => <li className={styles.conversation} key={item.id}><div><a href={item.href}>{item.title}</a><span className={styles.muted}>{item.messageCount === 0 ? "尚未开始" : `${item.messageCount} 条消息`} · {new Date(item.modified).toLocaleString()}</span></div>
            {!item.student && !project?.courseProjectId && <select aria-label={`整理对话：${item.title}`} value={item.projectId ?? ""} disabled={busy} onChange={(event) => { const projectId = event.target.value || null; void perform(async () => { const result = await projectAction({ action: "move", sessionId: item.id, projectId }); if (result.href) router.push(result.href); else await refresh(); }); }}><option value="">项目外 · 独立对话</option>{data.projects.map((folder) => <option value={folder.id} key={folder.id}>{folder.courseProjectId ? `课程 · ${folder.title}` : folder.title}</option>)}</select>}
          </li>)}</ul>
