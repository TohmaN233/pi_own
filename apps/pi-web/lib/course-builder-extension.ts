...
export function createCourseBuilderExtension(hostProvider = getCourseBuilderHost, executeCommand = courseBuilderCommand) {
 ...
 pi.registerTool({
  ...
  parameters:Type.Object({
   ...
   spec:Type.Optional(Type.Object({}, {additionalProperties:true,description:"add_material: {url OR path,name?,root?,purpose?}, expectedRevision=current project revision; save new files or captured website Markdown in an existing state.workspace.materialDirectories folder, never a new folder. Returns materials [{materialId,name,path}]. import_generated_asset: {path,lessonPlanId,sourcePath?,purpose} imports computed PNG/JPEG/PDF from state.workspace.outputDirectory for Beamer, not teaching references. delivery_route {kind,week?,session?,lessonPlanId?,deckId?,includeTeacherNotes?,requirements:[{text,verification:content|compile-review|checkpoint|materials,id?}]}: use kind=materials for standalone material acquisition; retain lesson/deck target for auxiliary imports. Host owns IDs. delivery_status returns saved requirement IDs, evidence readiness and the exact finishTemplate. delivery_finish checks contain only content requirements: {requirementId,quote}; material content also needs materialId and read_material offset. Host resolves compile-review/checkpoint/material registration records. Never guess requirement IDs or use a filename to prove captured content. Also used for visual specifications."})),
