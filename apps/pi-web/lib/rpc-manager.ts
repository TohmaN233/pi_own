...
type AgentSessionWrapperOptions = {
  ...
  requiredToolNames?: string[];
	profileSnapshot?: ResourceSnapshot;
};
...
const PROFILE_TRANSITION_READ_ONLY_COMMANDS = new Set([
	"get_state",
	"get_tools",
	"get_commands",
	"get_session_stats",
...
export interface RpcSessionStartOptions {
  ...
  /** Course selected before session construction; durable binding still happens in the route. */
  harnessCourseVersionId?: string;
  /** Internal prepared snapshot used while a bound session performs a warm switch. */
  harnessResourceSnapshot?: ResourceSnapshot;
  /** Build a verified candidate without exposing it in the live RPC registry. */
...
export function withExtensionTools(session: AgentSessionLike, toolNames: string[], requiredToolNames: string[] = []): string[] {
  if (toolNames.length === 0) return [...new Set(requiredToolNames)];
...
function assertHarnessRuntime(snapshot: ResourceSnapshot, session: AgentSessionLike): void {
  ...
  for (const name of expected) {
    if (!registered.has(name)) throw new Error(`Harness runtime is missing required tool ${name}.`);
  }
}
...
export class AgentSessionWrapper {
  ...
  private profileSnapshot?: ResourceSnapshot;
	private profileTransitionLocked = false;
  ...
  get streamingMessage() {
    const message = this.inner.agent.state?.streamingMessage;
    return this.groundedAnswerGate?.suppressSnapshot(message) ? undefined : message;
  }
  ...
  start(): void {
    this.unsubscribe = this.inner.subscribe((event: AgentEvent) => {
      if (event.type === "message_end") {
        // The AgentSession has already stored this object by reference. The gate
        // mutates it in place so SSE, agent state, and JSONL see one safe final.
        this.groundedAnswerGate?.enforceFinalMessage(event.message);
      }
      ...
      if (
        this.groundedAnswerGate?.isActive()
        && (
          (event.type === "message_start" && isAssistantMessageEvent(event))
          || (event.type === "message_update" && isAssistantMessageEvent(event))
        )
      ) return;
      this.emit(event);
  ...
  setActiveToolSelection(toolNames: string[]): void {
		if (this.profileSnapshot) {
			throw new Error("Course-bound sessions use their active Learning Harness profile; use the Profile selector instead of set_tools.");
		}
    this.inner.setActiveToolsByName(withExtensionTools(this.inner, toolNames, this.requiredToolNames));
    this.applyExactSystemPrompt();
  }
  ...
  setProfileTransitionLocked(locked: boolean): void {
		this.profileTransitionLocked = locked;
	}

	/** Atomically reserve an idle wrapper before a profile candidate is built. */
	tryAcquireProfileTransition(): boolean {
		if (this.profileTransitionLocked || this.activeMutatingCommands !== 0 || this.isSessionRunningForReplacement()) {
			return false;
		}
		this.profileTransitionLocked = true;
		return true;
	}

	releaseProfileTransition(): void {
		this.profileTransitionLocked = false;
	}
  ...
  async send(command: Record<string, unknown>): Promise<unknown> {
    ...
    try {
      ...
      case "prompt": {
        ...
        try {
			if (this.profileTransitionLocked) {
				throw new Error("Learning Harness profile transition is in progress; wait before changing this session.");
			}
      ...
      case "fork": {
        ...
        return this.withSessionReplacement("fork", async () => {
          ...
          if (!entry.parentId) {
            ...
            const childHeader = newManager.getHeader();
            if (!childHeader) throw new Error("Failed to create forked session header");
            writeFileSync(newSessionFile, `${JSON.stringify(childHeader)}\n`, { encoding: "utf8", flag: "wx" });
          } else {
...
declare global {
  ...
  var __piStartingSessionCwds: Map<string, number> | undefined;
	var __piHarnessProfileTransitionLocks: Set<string> | undefined;
}

function acquireHarnessProfileTransition(sessionId: string): () => void {
	const locks = globalThis.__piHarnessProfileTransitionLocks ??= new Set<string>();
	if (locks.has(sessionId)) throw new Error("A Learning Harness profile transition is already in progress for this session.");
	locks.add(sessionId);
...
export function getHarnessRuntimeVerification(sessionId: string, snapshot: ResourceSnapshot): {
	live: boolean;
	verified: boolean;
	activeTools: string[];
	...
	const activeTools = [...session.inner.getActiveToolNames()].sort();
	const verified = activeTools.length === expectedTools.length && activeTools.every((tool, index) => tool === expectedTools[index]);
	return {
		live: true,
		verified,
		activeTools,
		expectedTools,
		diagnostic: verified ? null : "Live Pi runtime active tools do not match the durable resource snapshot.",
	};
}
...
/**
 * Rebuild a bound learner runtime against a prepared immutable snapshot while
 * retaining the Pi session id and its single JSONL transcript.  The candidate
 * is deliberately kept out of the registry until the JSONL binding revision
 * and SQLite active snapshot have committed.
 */
export async function warmSwitchHarnessProfile(
	sessionId: string,
	prepared: { idempotencyKey: string; snapshot: ResourceSnapshot },
): Promise<AgentSessionWrapper> {
	const existing = getRpcSession(sessionId);
	if (!existing?.isAlive()) throw new Error("Profile switching requires a live course-bound Pi session.");
	const current = getLearningHarness().findCurrentSession(sessionId);
	if (!current) throw new Error("Profile switching requires a durable course-bound Harness session.");
	const release = acquireHarnessProfileTransition(sessionId);
	if (!existing.tryAcquireProfileTransition()) {
		release();
		getLearningHarness().abortPreparedProfileTransition(
			sessionId,
			prepared.idempotencyKey,
			current.snapshot.resourceSnapshotId,
		);
		throw new Error("Wait for the current Pi command to finish before switching profiles.");
	}
	let candidate: AgentSessionWrapper | null = null;
	let committed = false;
	let liveWrapper: AgentSessionWrapper = existing;
	const abortPending = () => {
		if (committed) return;
		getLearningHarness().abortPreparedProfileTransition(
			sessionId,
	...
	try {
		...
		candidate = candidateStart.session;
		candidate.setProfileTransitionLocked(true);
		if (candidateStart.realSessionId !== sessionId) {
			throw new Error("Prepared profile runtime changed the Pi session identity.");
		}
		await existing.shutdown();
		getLearningHarness().commitPreparedProfileTransition(
			candidate.inner.sessionManager,
			sessionId,
			prepared.idempotencyKey,
		);
		committed = true;
		...
		try { abortPending(); } catch (abortError) {
			console.error("[learning-harness] failed to abort uncommitted profile transition", abortError);
		}
		if (!committed) {
			try { existing.activateHarnessProfile(current.snapshot); } catch (restoreError) {
				console.error("[learning-harness] failed to restore old profile runtime", restoreError);
			}
		}
		if (candidate) {
			try { await candidate.shutdown(); } catch (shutdownError) {
				console.error("[learning-harness] failed to shut down rejected profile runtime", shutdownError);
			}
		}
		// If the old wrapper was already stopped before a pre-commit failure, reopen
		// the old durable snapshot. A post-commit error is left for startup
		// reconciliation, where the journal is authoritative.
		if (!committed && !getRpcSession(sessionId)?.isAlive()) {
			const sessionFile = existing.sessionFile;
			if (sessionFile && existsSync(sessionFile)) {
				try { await startRpcSession(sessionId, sessionFile, undefined); } catch (restoreError) {
					console.error("[learning-harness] failed to restore old profile runtime", restoreError);
				}
			}
		}
		...
		liveWrapper.releaseProfileTransition();
...
export async function startRpcSession(
  ...
  const { initialModel, allowInitialModelFallback, thinkingLevel, harnessCourseVersionId, harnessResourceSnapshot, deferRegister } = options;
  ...
  if (existing?.isAlive() && !deferRegister) return { session: existing, realSessionId: sessionId };

  const lockId = deferRegister ? `${sessionId}:prepared-profile-runtime` : sessionId;
  ...
  const sessionCwd = sessionManager.getCwd();
	// A reopened Pi JSONL is the commit authority for a prepared profile
	// transition. Reconcile it before choosing resources or creating services so
	// a journal-ahead restart cannot boot the old snapshot's tool allowlist.
	// An explicit candidate snapshot is intentionally exempt: it is built while
	// the old JSONL binding is still active and must not consume its pending row.
	const durableHarnessSession = harnessCourseVersionId || harnessResourceSnapshot
		? null
		: getLearningHarness().reconcileRuntimeSession(sessionManager);
	const harnessSnapshot = harnessResourceSnapshot ?? durableHarnessSession?.snapshot;
  ...
  const subagentLoadsResources = Boolean(
	...
	// A course-bound learner owns its tool policy through the snapshot, never
	// through Pi Web's generic coding-tool selection.
	const chatOnly = harnessEnabled || (selectedToolNames?.length === 0 && !subagentLoadsResources);
  const finishStartingSession = trackStartingSession(sessionCwd);
  const starting = (async () => {
    // Some extensions access the SDK's global theme even outside the terminal UI.
    if (!chatOnly || harnessEnabled) initTheme();
    const agentDir = getAgentDir();

    // Determine which tools to pass based on requested toolNames.
    // Since v0.68.0, session creation expects string[] tool names instead of Tool[] instances.
    let toolsOption: string[] | undefined = subagentResources?.tools;
    if (!subagentResources && (selectedToolNames !== undefined || harnessEnabled)) {
    ...
    const wrapper = new AgentSessionWrapper(inner, {
      ...
      ...(learningHarnessExtension ? {
        groundedAnswerGate: learningHarnessExtension.outboundGate,
			requiredToolNames: harnessSnapshot?.mode === "student-learn" ? ["submit_grounded_answer"] : [],
      } : {}),
